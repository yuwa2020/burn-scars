...
class ElevationDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # self.transformed_disaster_rbg = self.transforms(self.disaster_rgb)
        ## print("transformed_disaster_rbg: ", self.transformed_disaster_rbg.shape)
        # self.transformed_regular_rgb = self.transforms(self.regular_rgb)
        ## print("transformed_regular_rgb: ", self.transformed_regular_rgb.shape)

        # Merge Disaster and regular time RGB
        # self.transformed_rbg = np.concatenate((self.transformed_disaster_rbg, self.transformed_regular_rgb), axis = 0)
