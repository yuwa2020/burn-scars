...
async function pollBackendTask(taskId: string) {
    ...
    if (data.status === 'completed') {
        ...
        const predBuffer = await fetch(`http://127.0.0.1:5005/pred?testRegion=${testRegion}`).then(response => response.arrayBuffer());
...
async function retrainSession(event: Event) {
    ...
    if (data.status === 'success') {
        ...
        const predBuffer = await fetch(`http://127.0.0.1:5005/pred?predict=${0}&testRegion=${testRegion}`).then(response => response.arrayBuffer());
        console.log("arraybuffer: ", predBuffer)
