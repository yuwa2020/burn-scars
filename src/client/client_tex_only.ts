...
async function pollBackendTask(taskId: string) {
    const response = await fetch(`http://127.0.0.1:5005/check-status?taskId=${taskId}&testRegion=${sessionData.testRegion}`);
    ...
    if (data.status === 'completed') {
        ...
        const superpixelBuffer = await fetch(`http://127.0.0.1:5005/superpixel?recommend=${0}&testRegion=${sessionData.testRegion}`).then(response => response.arrayBuffer());
        ...
        const predBuffer = await fetch(`http://127.0.0.1:5005/pred?taskId=${taskId}&testRegion=${sessionData.testRegion}`).then(response => response.arrayBuffer());
        ...
        if (file.type == "image/png") {
            ...
            fr.onload = function (e) {
                ...
                image.onload = function() { 
                    ...
                    // Create a simple plane with image texture
                    const planeGeometry = new THREE.PlaneGeometry(image.width, image.height)
                    diffuseTexture.encoding = THREE.sRGBEncoding
                    const planeMaterial = new THREE.MeshBasicMaterial({ map: diffuseTexture })
                    ...
                    plane.position.set(image.width / 2, image.height / 2, 0)
                    // plane.position.set(0, 0, -100)
                    scene.add(plane)

                    ajax({
                        url: `http://127.0.0.1:5005/pred?predict=${1}&testRegion=${testRegion}&taskId=${student_id}&initial=${1}`,
                        type: 'GET',
                        // data: formData,
                        processData: false,
                        contentType: false,
                        ...
                        success: async function(data) {
                            ...
                            try {
                                const predBuffer = await data.arrayBuffer?.();
                                ...
                                // Wait for the image to load
                                imgPred.onload = () => {
                                    // console.log("imgPred loaded!");
                                    // console.log("imgPred.src =", imgPred.src);
                                    ...
                                    predictionTexture.image = imgPred;
                                    predictionTexture.needsUpdate = true; // saugat

                                    const predMaterial = new THREE.MeshBasicMaterial({
                                        map: predictionTexture,
                                        transparent: true,
                                        depthTest: false,
                                    });
                                
                                    predPlane = new THREE.Mesh(
                                        new THREE.PlaneGeometry(predCanvas.width, predCanvas.height),
                                        predMaterial
                                    );
                                    // predPlane.position.z = 0.1;
                                    predPlane.position.set(image.width / 2, image.height / 2, 0)
                                    scene.add(predPlane);

                                    // console.log(predCanvas.toDataURL());
                                ...
                                imgForest.onload = () => {
                                    ...
                                    context!.drawImage(imgForest, 0, 0);
                                    forestMapTexture.image = imgForest;
                                    forestMapTexture.needsUpdate = true;
                                    console.log("forestMapTexture: ", forestMapTexture)

                                    const forestMaterial = new THREE.MeshBasicMaterial({
                                        map: forestMapTexture,
                                        transparent: true,
                                        depthTest: false,
                                    });
                                
                                    forestPlane = new THREE.Mesh(
                                        new THREE.PlaneGeometry(annCanvas.width, annCanvas.height),
                                        forestMaterial
                                    );
                                    // forestPlane.position.z = 0.2;
                                    forestPlane.position.set(image.width / 2, image.height / 2, 0)
